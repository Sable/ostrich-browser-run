...
function bashToJavaScript(a) {
  ...
  if (a.indexOf('@') >= 0) {
    ...
    if (type[1] !== 'int32' && type[1] !== 'float64') {
...
app.ws('/socket', function (ws, req) {
  ...
  fs.writeFileSync(path.join(__dirname, 'input', 'args.js'), 'define(function () {\n' + args.map(indent).join('\n') + '\n' + '})');

  // Determine files to load
  var modules = ['/input/args.js', '/prng/ostrich-twister-prng-UMD.js'];
